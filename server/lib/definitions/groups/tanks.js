...
function makeAutoBranch(type, tier, options = {}) {
    ...
    let autoType = `auto${type.charAt(0).toUpperCase() + type.slice(1)}`;

    whitelist = options.whitelist ??= [];
    blacklist = options.blacklist ??= [];

    if (Class[type][`UPGRADES_TIER_${tier}`] == undefined) return;
    ...
    for (let i = 0; i < cleanUpgrades.length; i++) {
        let cleanUpgrade = cleanUpgrades[i];
        let typeUpgrade = ensureIsClass(cleanUpgrade);

        let label = typeUpgrade.LABEL;

        if (Class[`auto${cleanUpgrade.charAt(0).toUpperCase() + cleanUpgrade.slice(1)}`] == undefined) {
            Class[`auto${cleanUpgrade.charAt(0).toUpperCase() + cleanUpgrade.slice(1)}`] = makeAuto(cleanUpgrade, `Auto-${label}`);
        };
    };

    let autoUpgrades = cleanUpgrades.map(x => 'auto' + x.charAt(0).toUpperCase() + x.slice(1));

    if (options.megaTripleAuto) {
        const megaTripleAuto = ['megaAuto', 'tripleAuto'].map(x => x + type.charAt(0).toUpperCase() + type.slice(1));
        if (Class[`megaAuto${type.charAt(0).toUpperCase() + type.slice(1)}`] == undefined) {
            Class[`megaAuto${type.charAt(0).toUpperCase() + type.slice(1)}`] = makeAuto(type, `Mega Auto-${typeClass.LABEL}`, preset.makeAuto.mega);
            Class[`tripleAuto${type.charAt(0).toUpperCase() + type.slice(1)}`] = makeAuto(type, `Triple Auto-${typeClass.LABEL}`, preset.makeAuto.triple);
        };
        autoUpgrades.splice(0, 0, ...megaTripleAuto.map(x => x.endsWith('_AR') ? x.slice(0, -3) : x));
    };

    if (Class[autoType] == undefined) {
        Class[autoType] = makeAuto(type);
        Class[type][`UPGRADES_TIER_${tier}`].push(autoType);
    };

    return Class[autoType][`UPGRADES_TIER_${tier + 1}`] = autoUpgrades;
};
...
function makeAutoBranch(type, tier, options = {}) {
    ...
    megaAutoOptions = options.megaAutoOptions ??= preset.makeAuto.mega;
    tripleAutoOptions = options.tripleAutoOptions ??= preset.makeAuto.triple;
    ...
    for (let i = 0; i < cleanUpgrades.length; i++) {
        let cleanUpgrade = cleanUpgrades[i];
        let trueCleanUpgrade = cleanUpgrade;
        let typeUpgrade = ensureIsClass(cleanUpgrade);

        let label = typeUpgrade.LABEL

        for (let j = 0; j < filter.length; j++) {
            if (filter[j] && cleanUpgrade.slice(filter[j].length / -1) == filter[j]) {
                cleanUpgrade = cleanUpgrade.slice(0, filter[j].length / -1);
                cleanUpgrades[i] = cleanUpgrades[i].slice(0, filter[j].length / -1);
                label = label.slice(0, filter[j].length / -1);
        ...
        if (Class[`auto${cleanUpgrade.charAt(0).toUpperCase() + cleanUpgrade.slice(1)}`] == undefined) {
            Class[`auto${cleanUpgrade.charAt(0).toUpperCase() + cleanUpgrade.slice(1)}`] = makeAuto(trueCleanUpgrade, `Auto-${label}`, autoOptions);
        };
    };
    ...
    if (options.megaTripleAuto) {
        const megaTripleAuto = ['megaAuto', 'tripleAuto'].map(x => x + type.charAt(0).toUpperCase() + type.slice(1));
        if (Class[`megaAuto${type.charAt(0).toUpperCase() + type.slice(1)}`] == undefined) {
            Class[`megaAuto${type.charAt(0).toUpperCase() + type.slice(1)}`] = makeAuto(type, `Mega Auto-${typeClass.LABEL}`, megaAutoOptions);
            Class[`tripleAuto${type.charAt(0).toUpperCase() + type.slice(1)}`] = makeAuto(type, `Triple Auto-${typeClass.LABEL}`, tripleAutoOptions);
        };
        autoUpgrades.splice(0, 0, ...megaTripleAuto.map(x => x.endsWith('_AR') ? x.slice(0, -3) : x));
    };

    if (Class[autoType] == undefined) {
        Class[autoType] = makeAuto(type, `Auto-${typeClass.LABEL}`, autoOptions);
        Class[type][`UPGRADES_TIER_${tier}`].push(autoType);
    };
...
makeAutoBranch('basic', 1, {filter: ["Gun"]});
    makeAutoBranch('flankGuard', 2, {filter: ["Tank"]});
*/